...
import { DOWNLOADS_DIR, UWUMI_DIR } from '@/constants/config';
import { downloadRuntime } from '../runtimes';

// Transient throttle state — not persisted, not in Zustand
const progressThrottle = new Map<string, { time: number; pct: number }>();
const THROTTLE_MS = 300;
const THROTTLE_PCT = 1;
      ...
      startDownload: async (downloadId, onProgress) => {
        ...
        try {
          ...
          const handleSessionLog = (log: Log) => {
            ...
            const message = String(log.getMessage());

            // Parse progress on the worker runtime — pure string ops, no JS thread blocked
            runOnRuntimeAsync(
              downloadRuntime,
              (msg: string, dur: number) => {
                ...
                const sizeMatch = msg.match(/size=\s*(\d+)kB/);

                const parts = timeMatch[1].match(/(\d+):(\d+):(\d+\.?\d*)/);
                if (!parts) return null;
                const currentTime = parseInt(parts[1]) * 3600 + parseInt(parts[2]) * 60 + parseFloat(parts[3]);
                const percentage = dur > 0 ? Math.min(Math.max(Math.round((currentTime / dur) * 100), 0), 100) : 0;
              ...
              // Throttle: skip if pct hasn't moved enough and update is too soon
              const now = Date.now();
              const last = progressThrottle.get(dlId);
              if (last && Math.abs(parsed.percentage - last.pct) < THROTTLE_PCT && now - last.time < THROTTLE_MS) {
                return;
              }
              progressThrottle.set(dlId, { time: now, pct: parsed.percentage });
              ...
              const progress: DownloadProgress = {
                ...
                size: parsed.size || (dl.progress?.size ?? 0),
              };
